...
def read_squad_json_format(file, pre_evaluation_group):
    """
...
class DatasetReaderSQuAD11(DatasetReader):
    ...
    def load_entries(self):
        ...
        try:
            ...
            read_entries += read_squad_json_format(f, "train")

            # dev-v1.1.json file
            f = open(self.path + 'dev-v1.1.json')
            read_entries += read_squad_json_format(f, "dev")
...
class DatasetReaderSQuAD20(DatasetReader):
    ...
    def load_entries(self):
        ...
        try:
            ...
            # train-v1.1.json file
            f = open(self.path + 'train-v2.0.json')
            read_entries += read_squad_json_format(f, "train")

            # dev-v1.1.json file
            f = open(self.path + 'dev-v2.0.json')
            read_entries += read_squad_json_format(f, "dev")
